...
from family_of_symmetries import generate_family_symmetries
...
def main(n):
    ...
    while sol < 13:
        ...
        if len(result) == 0: # The first solution
            if not has_clashes(permutation):
                result.append(permutation)
                print("Solution is {0} in {1} tries.".format(permutation, tries))
                sol += 1
                tries = 0 # Found solution, reset tries for next solution

        else: # From the second solution and more
            if not has_clashes(permutation):
                for i in result:
                    if permutation not in result \
                    and permutation not in generate_family_symmetries(i):
                        result.append(permutation)
                        print("Solution is {0} in {1} tries.".format(permutation, tries))
                        sol += 1
                        tries = 0 # Found solution, reset tries for next solution
